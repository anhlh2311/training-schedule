...
- **Duplicate notifications (same title/body twice) vs none at all:** On **web**, FCM **`notification`** payloads do **not** reliably auto-display in all browsers; skipping `showNotification()` in the service worker can yield **no** notification. The app therefore sends **data-only** messages (`title`, `body`, `type`, `url` as strings in `data`) from [api/notify.ts](mdc:api/notify.ts) and [api/cron/process-batch.ts](mdc:api/cron/process-batch.ts), and [scripts/generate-firebase-sw.cjs](mdc:scripts/generate-firebase-sw.cjs) always calls `showNotification()` from `onBackgroundMessage` — a single display path. Foreground tabs use `onMessage` in [registerFcmToken.ts](mdc:src/lib/registerFcmToken.ts) after token registration.
